#!/bin/bash
# 
# Developed by Fred Weinhaus 5/1/2009 .......... revised 12/31/2018
#
# ------------------------------------------------------------------------------
# 
# Licensing:
...
# with the image. Values are floats such that 10<=wrap<=100 or 0. Default=50. Use 
# wrap=0 to preserve aspect ratio of input as it is wrapped around the cylinder.
# 
...
if [ "$mode" = "vertical" ]; then
	...
	# if wrap = 0, then wrap = ratio of aspect ratios
	[ "$wrap" = "0" ] && wrap=`convert xc: -format "%[fx:100*($width/$height)/(2*pi*$radius/$length)]" info:`
	...
	[ "$wrap" = "0" ] && wrap=`convert xc: -format "%[fx:100*($height/$width)/(2*pi*$radius/$length)]" info:`
	pwidth=$width
